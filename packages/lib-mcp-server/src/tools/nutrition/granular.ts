...
const NutritionModificationChangesSchema = z.object({
  ...
  targetCalories: z
    ...
    .positive()
    .optional()
    .describe('Target calories. Example: "2000 kcal" → 2000'),
  targetMacros: z
    .object({
  ...
  // Food fields
  quantity: z.number().positive().optional().describe('Food quantity. Example: "200g" → 200'),
  unit: z.string().optional().describe('Unit (g, ml, pz)'),
});

const nutritionApplyModificationParams = z
  .object({
    planId: z.string(),
...
- "200g di riso": action=update_food, changes={quantity: 200}`,
  ...
  execute: async (args, _context: McpContext) => {
    ...
    let mealIndex = -1;
    if (target.mealName) {
      mealIndex = day.meals.findIndex(
        (m) => m.name.toLowerCase() === target.mealName?.toLowerCase()
      );
    } else {
    ...
    switch (action) {
      case 'update_meal': {
        ...
        if (changes?.targetMacros)
          meal.targetMacros = { ...meal.targetMacros, ...changes.targetMacros };
      ...
      case 'update_food': {
        ...
        if (foodIndex === -1)
          throw new Error(`Alimento "${target.foodName || target.foodId}" non trovato`);
      ...
      case 'add_food': {
        ...
        // Calculate macros
        const macrosBase = foodItem.macrosPer100g as Record<string, number>;
